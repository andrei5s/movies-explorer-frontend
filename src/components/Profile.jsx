  ...
  return (
    ...
    <section className="profile">
      ...
      <form className="profile__form" id="edit">
          ...
          <input className="form__input" value={values?.name || ""} type="name" name="name" placeholder="Имя" minLength="2" maxLength="30" required />        
        {errors?.name && <span className="error form__input-error">{errors.name}</span>}
        <div className="form__line"></div>
          <label className="form__indicator">E-mail</label>
          <input className="form__input" value={values?.email || ""}
